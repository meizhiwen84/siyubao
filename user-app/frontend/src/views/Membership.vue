      ...
      <div class="bg-white rounded-xl shadow-xl w-full max-w-lg border border-gray-200">
        ...
        <div class="p-5">
          ...
          <div class="space-y-4">
            <div>
              ...
              <select v-model="upgradeForm.platform" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" @change="onPlatformChange">
                <option 
            ...
            <div v-if="paymentConfig" class="text-center">
              ...
              <img
                v-if="upgradeForm.platform === 'wechat' && paymentConfig.wechatQr && paymentConfig.wechatQr.url"
                :src="paymentConfig.wechatQr.url + '?t=' + qrUpdateTime"
                alt="微信收款码"
                class="max-w-48 max-h-48 border border-gray-200 rounded-lg mx-auto"
              />
              <img
                v-else-if="upgradeForm.platform === 'alipay' && paymentConfig.alipayQr && paymentConfig.alipayQr.url"
                :src="paymentConfig.alipayQr.url + '?t=' + qrUpdateTime"
                alt="支付宝收款码"
                class="max-w-48 max-h-48 border border-gray-200 rounded-lg mx-auto"
              />
              <div v-if="(upgradeForm.platform === 'wechat' && (!paymentConfig.wechatQr || !paymentConfig.wechatQr.url)) || (upgradeForm.platform === 'alipay' && (!paymentConfig.alipayQr || !paymentConfig.alipayQr.url))" class="text-sm text-gray-500">
                请联系客服获取收款码
              </div>
            </div>
        ...
        <div class="px-5 py-4 border-t border-gray-200 flex space-x-3">
          ...
          <button 
            ...
            :disabled="upgradeSubmitting || !upgradeForm.transactionId || !upgradeForm.transactionId.trim()"
            @click="submitUpgrade"
          >
...
const qrUpdateTime = ref(0)
const transactionId = ref('')

watch(transactionId, (newVal) => {
  upgradeForm.value.transactionId = newVal
})
